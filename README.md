...
For long-running sessions, issue a periodic checkpoint by also passing an
explicit anchor input and disclosure. The normal parent comparison remains, and
the checkpoint adds one anchor comparison without replaying the full chain:
...
derived = issue_measurement_receipt(
    ...
    previous_input_receipt=previous_receipt,
    previous_disclosure=previous_disclosure,
    checkpoint_anchor_input_receipt=run_start_receipt,
    checkpoint_anchor_disclosure=run_start_disclosure,
)
```
