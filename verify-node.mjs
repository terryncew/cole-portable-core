...
function verifyDerived(
  ...
  checkpointAnchorReceipt = null,
  checkpointAnchorDisclosure = null,
) {
  ...
  const checkpointAnchor = checkpointAnchorReceipt === null ? null : validateInput(checkpointAnchorReceipt, checkpointAnchorDisclosure);
  exact(derived, ["kind", "receipt_version", "algorithm_id", "canonicalization_id", "spec_uri", "attestation", "input_receipt_hash", "input_trace_id", "previous_input_receipt_hash", "checkpoint", "profile", "measurement", "payload_hash", "signature"]);
  ...
  if (encode(evaluate(current.graph, current.signal, previous?.graph ?? null, derived.profile)) !== encode(derived.measurement)) return false;
  if (derived.checkpoint === null) return checkpointAnchorReceipt === null && checkpointAnchorDisclosure === null;
  if (checkpointAnchorReceipt === null || checkpointAnchorDisclosure === null) return false;
  exact(derived.checkpoint, ["anchor_input_receipt_hash", "anchor_measurement"]);
  if (derived.checkpoint.anchor_input_receipt_hash !== checkpointAnchorReceipt.payload_hash) return false;
  return encode(evaluate(current.graph, current.signal, checkpointAnchor.graph, derived.profile)) === encode(derived.checkpoint.anchor_measurement);
}

function read(path) { return parseJsonStrict(fs.readFileSync(path, "utf8")); }

if (process.argv.length !== 5 && process.argv.length !== 7 && process.argv.length !== 9) {
  console.error("usage: node verify-node.mjs INPUT DISCLOSURE MEASUREMENT [PREVIOUS_INPUT PREVIOUS_DISCLOSURE [CHECKPOINT_ANCHOR_INPUT CHECKPOINT_ANCHOR_DISCLOSURE]]");
  process.exit(2);
}
try {
  ...
  const checkpointAnchorReceipt = process.argv.length === 9 ? read(process.argv[7]) : null;
  const checkpointAnchorDisclosure = process.argv.length === 9 ? read(process.argv[8]) : null;
  if (!verifyDerived(derived, currentReceipt, currentDisclosure, previousReceipt, previousDisclosure, checkpointAnchorReceipt, checkpointAnchorDisclosure)) throw new Error("verification failed");
